...
const ProductsListItem: React.FC<ProductsListItemProps> = ({ item }) => {
  ...
  const isFavorite = favoritesProducts.some(favorite => favorite.id === item.id)
  ...
  return (
    <li className='relative transition-transform duration-300 hover:scale-[1.03] focus:scale-[1.03]'>
      <Link
        ...
        <div className='flex w-full flex-col justify-start gap-2 rounded-b-2xl bg-white-dis p-2'>
          ...
          <p
            className={`flex items-baseline gap-1 font-exo_2 ${
              item.attributes.discount ? 'text-[red]' : 'text-black-dis'
            }  text-lg uppercase`}
          >
